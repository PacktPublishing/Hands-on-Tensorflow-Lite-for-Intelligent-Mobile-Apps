...
# Video 5-4.2: Adapting and debugging the model
#
...
def getModel():
	...
	summary_op = tf.summary.image("plot", xi,4)
	...
	re_pre_act = tf.reshape(pre_act,[-1,TARGET_DIM-2,TARGET_DIM-2,1])
	#re_batch_n = tf.reshape(batch_n,[-1,TARGET_DIM-2,TARGET_DIM-2,1])
	re_act_ = tf.reshape(act_,[-1,TARGET_DIM-2,TARGET_DIM-2,1])

	summary_op = tf.summary.image("feed_forward",re_pre_act,4)
	#summary_op = tf.summary.image("batch_norm", re_batch_n,4)
	summary_op = tf.summary.image("activation", re_act_,4)
...
with tf.Session() as sess:
	...
	# There will be only 10 steps displayed.
	# To change it, go to tensorboard/backend/application.py
	# and change DEFAULT_TENSOR_SIZE_GUIDANCE
	...
	for i in range(epochs):
		...
		for j in range(0,data["train"][0].shape[0],b_size):
			...
			[la,c,summary]=sess.run([optimizer,cost,merged], feed_dict={x: x_raw, x_sum1: x_raw_sum1, x_sum2: x_raw_sum2, y: y_raw})
